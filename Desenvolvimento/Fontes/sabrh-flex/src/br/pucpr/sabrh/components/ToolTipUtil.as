package br.pucpr.sabrh.components
{
	...
	{	
		...
		public static function createToolTip(target:UIComponent,msg:String,icon:Class=null,isError:Boolean=false,position:String=BOTTOM,timerHide:Number=5000):void{
			...
			//Estou trabalhando apenas com dois temas, um pra erro outra pra informaÃ§Ãµes,
			//Ã© possÃ­vel criar um tema para cada tipo de mensagem exemplo(Ajuda, InformaÃ§Ã£o,Aviso,Pergunta etc)
			if(isError){
				//Verifica se Ã© a versÃ£o do sdk Ã© menor que o Flex 4
				if (FlexVersion.CURRENT_VERSION < 0x04000000){
					currentTarget.setStyle("themeColor", currentTarget.getStyle("errorColor"));//Flex 3
				}else{
					currentTarget.setStyle("focusColor", currentTarget.getStyle("errorColor"));//Flex 4 agora Ã© focusColor! 
				}
			...
			if(icon){//Insere o Ã­cone
				var img:Image = new Image();
				img.source = icon;
				var _textField:TextField = toolTip.getChildAt(1) as TextField;
				toolTip.addChild(img);
				_textField.text = "         "+msg;//EspaÃ§o para o texto nÃ£o ficar sobre a Ã­cone
				img.width = 25;
				img.height = 25;
				img.x =  _textField.x-2;
				img.y = _textField.y-2;
			}
			startHideTimer(timerHide);//Inicializa o contador que irÃ¡ remover o tooltip
		...
		//Faz uma animaÃ§Ã£o com o tooltip antes de destrui-lo
